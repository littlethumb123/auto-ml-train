...
4. **Novelty check (required when consecutive_discards ≥ 2):** Read `state/UNEXPLORED_TECHNIQUES.md`.
   List every technique class with `Status = Unexplored` AND `Expected Δ > noise_floor`.
   You MUST either (a) select one of these as your plan, or (b) write one explicit sentence per
   class explaining why it is not appropriate given the current evidence.
   If `consecutive_discards ≥ 3` (C2 just resolved): step 4 takes priority over the trigger
   table in step 5 — you MUST select from Unexplored unless none remain or all are excluded
   with stated reasoning.
5. Consult `STRATEGY_GUIDE.md §1` (evidence-conditioned triggers): scan every condition
   row and identify which are true. The first unresolved implication is the default next move.
   If the "stuck → novelty" trigger fires (see STRATEGY_GUIDE §1 last row), the novelty
   check in step 4 overrides the default trigger selection.
6. **Pre-selection reasoning (required):** Enumerate 2–3 candidate action types suggested
   by steps 4–5. For each candidate, write one sentence estimating expected Δ using
   ...
   triggers. If you override the default implication from step 5, state why.
7. Choose ONE hypothesis that (a) does not retry a dead-end, (b) is testable within the
   time budget in `EVAL_PROTOCOL.md`, (c) respects the `DATA_CONTRACT.md` column whitelist.
8. Decide the `action_type` (see `EVAL_PROTOCOL.md` for the allowed list).
9. If the plan needs `experiment_helpers/<exp_id>/` files, list them explicitly in §Plan.
10. Write `state/NEXT_EXPERIMENT.md` per the schema in §2.3.4.
