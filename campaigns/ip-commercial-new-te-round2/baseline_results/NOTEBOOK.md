...
- **TEST SET EVALUATION (r50):** test_lift@1%=22.484 vs val=23.260 (gap=+0.777, 1.46× SE). Generalization is acceptable — gap concentrates in extreme top-1% tail where val-based weight optimization has most leverage. AUC-ROC generalizes near-perfectly (0.857→0.856). Ensemble adds +0.254 lift on test vs best individual (LGBM_h=22.230). All 7 base models individually generalize within ±0.4 lift points.

- **CAMPAIGN CONCLUSION (r50):** 50/100 rounds used. Champion: r48 DE-optimized 7-model ensemble. The val-test gap is moderate but not alarming — it reflects the inherent noise in the top-1% tail (7,526 test positives in 1.68M test rows at 0.45% prevalence vs 5,826 val positives in 752K val rows at 0.77% prevalence). If the campaign were to resume, the most promising direction would be re-evaluating previously-discarded base model changes (r33-r45) under DE weight optimization, since those were evaluated against the suboptimal NM weights.
