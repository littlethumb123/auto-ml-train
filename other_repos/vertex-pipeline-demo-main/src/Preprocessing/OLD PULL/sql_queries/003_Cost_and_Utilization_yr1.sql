-- Cost and Utilization Year 1 Query
-- Creates multiple tables for ED cases, IP cases, OP visits, and cost/utilization metrics
-- Variables: GCP_PROJECT, GCP_DB, PREFIX, OWNER, COST_CENTER, DEFAULT_EXP, ST

-- Drop existing tables
DROP TABLE IF EXISTS `{GCP_PROJECT}.{GCP_DB}.{PREFIX}_ed_cases_yr1`;
DROP TABLE IF EXISTS `{GCP_PROJECT}.{GCP_DB}.{PREFIX}_ed_yr1`;
DROP TABLE IF EXISTS `{GCP_PROJECT}.{GCP_DB}.{PREFIX}_ip_cases_yr1`;
DROP TABLE IF EXISTS `{GCP_PROJECT}.{GCP_DB}.{PREFIX}_ip_yr1`;
DROP TABLE IF EXISTS `{GCP_PROJECT}.{GCP_DB}.{PREFIX}_op_yr1`;
DROP TABLE IF EXISTS `{GCP_PROJECT}.{GCP_DB}.{PREFIX}_med_claims_flag_yr1`;
DROP TABLE IF EXISTS `{GCP_PROJECT}.{GCP_DB}.{PREFIX}_other_cost_utilization_yr1`;

-- Create ED cases table
CREATE OR REPLACE TABLE `{GCP_PROJECT}.{GCP_DB}.{PREFIX}_ed_cases_yr1`
...
-- Create ED summary table
CREATE TABLE `{GCP_PROJECT}.{GCP_DB}.{PREFIX}_ed_yr1`
OPTIONS (labels = [("owner", "{OWNER}"),("cost_center", "{COST_CENTER}")]
         , expiration_timestamp = TIMESTAMP_ADD(CURRENT_TIMESTAMP(), {DEFAULT_EXP}))
AS
SELECT 
...
FROM 
    (SELECT DISTINCT asdb_member_key, asdb_plan_key, index_dt FROM `{ST}`) AS st
LEFT JOIN 
    `{GCP_PROJECT}.{GCP_DB}.{PREFIX}_ed_cases_yr1` AS mc
        ON st.asdb_member_key = mc.asdb_member_key
...
-- Create IP cases table
CREATE OR REPLACE TABLE `{GCP_PROJECT}.{GCP_DB}.{PREFIX}_ip_cases_yr1`
OPTIONS (labels = [("owner", "{OWNER}"),("cost_center", "{COST_CENTER}")]
         , expiration_timestamp = TIMESTAMP_ADD(CURRENT_TIMESTAMP(), {DEFAULT_EXP}))
AS
SELECT 
...
CREATE TABLE  `{GCP_PROJECT}.{GCP_DB}.{PREFIX}_ip_yr1`
OPTIONS (labels = [("owner", "{OWNER}"),("cost_center", "{COST_CENTER}")]
         , expiration_timestamp = TIMESTAMP_ADD(CURRENT_TIMESTAMP(), {DEFAULT_EXP}))
AS
WITH acute AS (
    ...
    FROM  
        `{GCP_PROJECT}.{GCP_DB}.{PREFIX}_ip_cases_yr1`
    WHERE 
        ip_type = "Acute"
    GROUP BY 
        asdb_member_key
...
-- Create OP visits table
CREATE TABLE  `{GCP_PROJECT}.{GCP_DB}.{PREFIX}_op_yr1`
...
-- Create med claims flag table
CREATE TABLE `{GCP_PROJECT}.{GCP_DB}.{PREFIX}_med_claims_flag_yr1`
OPTIONS (labels = [("owner", "{OWNER}"),("cost_center", "{COST_CENTER}")]
         , expiration_timestamp = TIMESTAMP_ADD(CURRENT_TIMESTAMP(), {DEFAULT_EXP}))
AS
WITH clm AS (
...
-- Create other cost utilization summary table
CREATE TABLE `{GCP_PROJECT}.{GCP_DB}.{PREFIX}_other_cost_utilization_yr1`
OPTIONS (labels = [("owner", "{OWNER}"),("cost_center", "{COST_CENTER}")]
         , expiration_timestamp = TIMESTAMP_ADD(CURRENT_TIMESTAMP(), {DEFAULT_EXP}))
AS
SELECT
...
FROM 
    (SELECT DISTINCT asdb_member_key, asdb_plan_key, index_dt FROM `{ST}`) AS st
LEFT JOIN 
    `{GCP_PROJECT}.{GCP_DB}.{PREFIX}_med_claims_flag_yr1` AS clm
        ON st.asdb_member_key=clm.asdb_member_key
        AND st.index_dt = clm.index_dt
GROUP BY 
    st.asdb_member_key
