...
WITH baby AS (
    ...
    FROM 
        (
            ...
            WHERE 1 = 1
                AND CAST(dob AS DATE) BETWEEN "2022-06-01" AND "2024-05-31"
        ) AS mem
    LEFT JOIN
        (
            SELECT
            ...
            FROM
                `{GCP_PROJECT}.{GCP_DB}.{PREFIX}_all_diagnoses` --change to asdb_diagnoses once created
            WHERE 1 = 1
...
, gest_age1 AS (
    ...
    FROM
        mom
    LEFT JOIN
        `{GCP_PROJECT}.{GCP_DB}.{PREFIX}_all_diagnoses` AS clm --change to asdb_diagnoses once created
