-- Medical Claims Query - OPTIMIZED
-- Single scan of ASDB_CLM_DATA_STAGE for full 24-month window
-- Creates _med_claims_all with year_flag column (1=yr1, 2=yr2)
-- Downstream queries filter by year_flag
-- Variables: GCP_PROJECT, GCP_DB, PREFIX, OWNER, COST_CENTER, DEFAULT_EXP, ST

DROP TABLE IF EXISTS `{GCP_PROJECT}.{GCP_DB}.{PREFIX}_med_claims_all`;
...
SELECT
       ...
       , CASE 
           WHEN CAST(clm.asdb_incurred_dt AS DATE) >= DATE_SUB(st.index_dt, INTERVAL 12 MONTH) THEN 1
           ELSE 2
         END AS year_flag
FROM 
...
WHERE 1 = 1
         AND CAST(asdb_incurred_dt AS DATE) BETWEEN DATE_SUB(st.index_dt, INTERVAL 24 MONTH) AND DATE_SUB(st.index_dt, INTERVAL 1 DAY)
