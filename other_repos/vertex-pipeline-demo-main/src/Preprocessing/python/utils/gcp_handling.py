...
def create_custom_training_job_from_component(
    ...
    persistent_resource_id: str = None,  # _placeholders.PERSISTENT_RESOURCE_ID_PLACEHOLDER,
  ...
  Args:
   ...
   Returns:
    ...
    cj_pipeline_spec = json_format.MessageToDict(
        custom_job.component.custom_training_job.pipeline_spec
    )
    ...
    user_component_spec = user_pipeline_spec["components"][user_component_spec_key]
    ...
    if component_spec.description:
        component_description = textwrap.dedent(
            f"""
            ...
            {custom_job.component.custom_training_job.description}
            """
        )
