-----------------------------
--- custom pregnancy labs ---
-----------------------------
-- Uses {INDEX_DT} as the reference date for production scoring
...
WITH labresults_daily AS (
    SELECT DISTINCT 
        ...
        , CASE WHEN TRIM(d.loinc_class_cd)= "TRIGLYC" 
            AND CAST(b.service_start_dts AS DATE) < DATE_SUB({INDEX_DT}, INTERVAL st.gest_age WEEK)
    ...
    INNER JOIN 
        `edp-prod-hcbstorage.edp_hcb_mdcd_core_srcv.ASDB_MEMBER` AS xref
            ON st.asdb_member_key = xref.asdb_member_key
    INNER JOIN 
    ...
    WHERE 1 = 1 
        AND {INDEX_DT} >= DATE_SUB(CAST(b.service_start_dts AS DATE), INTERVAL 36 MONTH)
...
SELECT 
    ...
    -- ASDB-only lab columns (not available in EDW)
    , MAX(CASE WHEN lab_bp = 1 THEN result_value ELSE NULL END) AS lab_bp
    , MAX(CASE WHEN lab_hemoglob = 1 THEN result_value ELSE NULL END) AS lab_hemoglob
    , MAX(CASE WHEN lab_protein = 1 THEN result_value ELSE NULL END) AS lab_protein
    , MAX(CASE WHEN lab_WBC = 1 THEN result_value ELSE NULL END) AS lab_WBC
    , MAX(CASE WHEN lab_GFR = 1 THEN result_value ELSE NULL END) AS lab_GFR
FROM 
    labresults_daily
GROUP BY 
    asdb_member_key
