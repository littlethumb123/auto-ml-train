...
SELECT DISTINCT
    ...
    , CASE 
        WHEN CAST(rx.asdb_incurred_dt AS DATE) >= DATE_SUB(st.rpt_end_dt, INTERVAL 12 MONTH) THEN 1
        ELSE 2
      END AS year_flag
FROM 
...
WHERE 
    rx.ClaimType = "P"
    AND CAST(rx.asdb_incurred_dt AS DATE) BETWEEN DATE_SUB(st.rpt_end_dt, INTERVAL 24 MONTH) AND DATE_SUB(st.rpt_end_dt, INTERVAL 1 DAY);

-- Step 2: Create pivoted summary table with yr1/yr2 columns (one row per member)
