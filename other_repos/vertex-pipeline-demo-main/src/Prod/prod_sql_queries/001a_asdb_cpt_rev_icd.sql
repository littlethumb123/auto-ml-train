...
--- Filtered to only include members in the scoring cohort                            ---
-----------------------------------------------------------------------------------------
...
AS
WITH cohort AS (
    SELECT DISTINCT asdb_member_key, claimid FROM (
        SELECT asdb_member_key, claimid 
        FROM `edp-prod-hcbstorage.edp_hcb_mdcd_core_srcv.ASDB_CLM_DATA_STAGE`
        WHERE asdb_member_key IN (SELECT asdb_member_key FROM `{GCP_PROJECT}.{GCP_DB}.{PREFIX}_st`)
    )
),
icd AS (
    SELECT
    ...
    FROM
        `edp-prod-hcbstorage.edp_hcb_mdcd_core_srcv.ASDB_ASDB_CLAIMDIAG` AS diag
    INNER JOIN
        cohort ON diag.claimid = cohort.claimid
    LEFT JOIN
        (SELECT ICD9_DX_GROUP_NBR, ICD9_DX_CD FROM `edp-prod-hcbstorage.edp_hcb_core_cnsv.ICD9_DIAGNOSIS`) AS grp
          ON TRIM(UPPER(diag.codeid)) = TRIM(UPPER(grp.ICD9_DX_CD))
)
SELECT 
 asdb_plan_key
 , claimid
...
FROM 
    `edp-prod-hcbstorage.edp_hcb_mdcd_core_srcv.ASDB_CLAIMICDPROCSUMMARY` AS proc
INNER JOIN
    cohort ON proc.claimid = cohort.claimid
...
AS
WITH cohort AS (
    SELECT DISTINCT asdb_member_key FROM `{GCP_PROJECT}.{GCP_DB}.{PREFIX}_st`
)
SELECT DISTINCT
    clm.asdb_member_key
...
FROM 
    (
        ...
        INNER JOIN
            cohort ON clm.asdb_member_key = cohort.asdb_member_key
        LEFT JOIN
            (SELECT ICD9_DX_GROUP_NBR, ICD9_DX_CD FROM `edp-prod-hcbstorage.edp_hcb_core_cnsv.ICD9_DIAGNOSIS`) AS grp
