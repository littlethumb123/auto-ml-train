...
-- Variables: GCP_PROJECT, GCP_DB, PREFIX, OWNER, COST_CENTER, DEFAULT_EXP, INDEX_DT
...
SELECT 
    ...
    , CASE WHEN CAST(mc.asdb_incurred_dt AS DATE) >= DATE_SUB(st.rpt_end_dt, INTERVAL 12 MONTH) THEN 1 ELSE 2 END AS year_flag
FROM 
    (SELECT DISTINCT asdb_member_key, asdb_plan_key, rpt_end_dt FROM `{GCP_PROJECT}.{GCP_DB}.{PREFIX}_st`) AS st
INNER JOIN 
...
WHERE 
    CAST(mc.asdb_incurred_dt AS DATE) BETWEEN DATE_SUB(st.rpt_end_dt, INTERVAL 24 MONTH) AND DATE_SUB(st.rpt_end_dt, INTERVAL 1 DAY)
    AND CAST(mc.asdb_coe_id AS INT64) = 20100
    AND event_ct=1;

---------------------------------
-- STEP 2: ED Summary - PIVOTED with yr1/yr2 columns
---------------------------------
...
---------------------------------
-- STEP 3: IP Cases (single scan of ASDB_ICE_IP for 24 months)
---------------------------------
...
SELECT 
    ...
    , CASE WHEN CAST(mc.asdb_event_start_dt AS DATE) >= DATE_SUB(st.rpt_end_dt, INTERVAL 12 MONTH) THEN 1 ELSE 2 END AS year_flag
FROM
    (SELECT DISTINCT asdb_member_key, asdb_plan_key, rpt_end_dt FROM `{GCP_PROJECT}.{GCP_DB}.{PREFIX}_st`) AS st
INNER JOIN 
    `edp-prod-hcbstorage.edp_hcb_mdcd_core_srcv.ASDB_ICE_IP` AS mc
        ...
        AND st.asdb_plan_key=mc.asdb_plan_key
WHERE 
    CAST(mc.asdb_event_start_dt AS DATE) BETWEEN DATE_SUB(st.rpt_end_dt, INTERVAL 24 MONTH) AND DATE_SUB(st.rpt_end_dt, INTERVAL 1 DAY)
    AND mc.event_ct=1;

---------------------------------
-- STEP 4: IP Summary - PIVOTED with yr1/yr2 columns
---------------------------------
...
---------------------------------
-- STEP 5: OP Visits - PIVOTED with yr1/yr2 columns
---------------------------------
...
WITH clm AS(
    SELECT 
        *
        , CASE WHEN ROW_NUMBER() OVER(PARTITION BY asdb_member_key, asdb_plan_key, asdb_svc_prov_key, asdb_incurred_dt, year_flag) = 1 THEN 1 ELSE 0 END AS op_ct
...
---------------------------------
-- STEP 6: Med Claims Flag with year_flag (from _med_claims_all)
-- This stays claim-level for downstream use by 010_preventative
---------------------------------
