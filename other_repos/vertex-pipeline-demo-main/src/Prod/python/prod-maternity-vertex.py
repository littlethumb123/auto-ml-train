...
user_constants = {
    ...
    "COSTCENTER": "13070",
    "TENANT": "hcm-cm-de",
    "service_account": "gchcb-hcm-cm-de-onppp@anbc-prod-hcm-cm-de.iam.gserviceaccount.com",
    ...
    "compute_dataset": "hcm_cm_de_beam_prod_hcm_cm_de",
    "expiration_days": 2,
...
PIPELINE_ROOT = os.getenv("PIPELINE_ROOT", "gs://hcm-cm-de-code-hcb-prod/vertex-test/")
...
def vertex_batch_predict_bigquery_component(
    ...
    if expiration_days:
        ...
        options=labels
    query = f"CREATE OR REPLACE TABLE {output_table} OPTIONS({options}) AS SELECT asdb_member_key, CURRENT_DATE() as run_dt, index_dt, prediction as ptl_riskscore FROM `{temp_output_table}`"
...
def prod_bigquery_pipeline(
    ...
    batch_pred_job = vertex_batch_predict_bigquery_component(
        ...
        output_table="anbc-hcb-prod.cm_medicaid_hcb_prod.medicaid_hr_maternity_ptl_predict",
...
if __name__ == "__main__":
    ...
    schedules = aiplatform.PipelineJobSchedule.list(
        filter='display_name="maternity-prod-scoring-pipeline"',
    ...
    display_name="maternity-prod-scoring-pipeline",
    cron="TZ=America/New_York 0 15 * * 4",
