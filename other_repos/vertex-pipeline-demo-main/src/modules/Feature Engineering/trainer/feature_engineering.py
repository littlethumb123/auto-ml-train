"""Feature classification and encoding for the Feature Engineering pipeline.

Feature type codes
------------------
0  categorical  — object/category dtype, or numeric with few unique values
1  continuous   — numeric with many unique values; columns matching 'emb' or '_amt'
2  binary       — exactly 2 unique values from a known set ({0,1}, {Y,N}, etc.)
3  datetime     — detected by dtype, column-name keywords, or value patterns

Public API
----------
``auto_classify_features(df, ...)``
    Scans every column in *df* and assigns one of the four type codes above.
    Respects manual overrides and an exclusion list. Samples up to
    *sample_size* rows for speed on large DataFrames.

``fit_one_hot_encoders(df, categorical_features, min_occurrence)``
    Fits a ``sklearn.OneHotEncoder`` for each categorical column.
    Only categories with at least *min_occurrence* rows in the training set
    are retained; rare categories are handled as unknown at transform time.
    Returns ``(encoders, categories_to_keep_per_feature)``.

``apply_one_hot_encoding(df, categorical_features, encoders, categories_to_keep_per_feature)``
    Applies the fitted encoders to *df*. Rows whose value is not in
    *categories_to_keep* are zero-filled in the resulting OHE columns.
    The original categorical column is dropped and replaced with the
    encoded columns.
"""
import pandas as pd
