"""Main entry point for the Feature Engineering pipeline on Vertex AI.

This script is submitted as a Vertex AI Custom Training Job. All settings
(SQL queries, feature classification config, RFECV config, etc.) are passed
via CLI arguments so no code changes are needed between runs.

Pipeline steps
--------------
1.  Parse CLI arguments (complex configs passed as JSON or base64-encoded JSON)
2.  Set up GCP environment variables and random seeds
3.  Derive runtime constants for SQL variable substitution
4.  Initialise BigQuery client
5.  Load training and test DataFrames from BigQuery
6.  Fill missing values (embeddings & numerics → 0, strings → '')
7.  Drop low-variance numeric features below variance_threshold
8.  Auto-classify features into types (categorical / continuous / binary / datetime)
9.  Fit OneHotEncoders on training data (only categories with ≥ min_occurrence rows)
10. Apply fitted encoders to both train and test; sanitise column names
11. Prepare X_train, y_train, X_test, y_test (numeric columns only)
12. Train a baseline XGBoost model; compute ROC-AUC + percentile metrics
13. Run undersampling experiments across configurable ratios and seeds
14. Print label distribution for train and test sets
15. Run RFECV to select the optimal feature subset
16. Save selected feature names to a timestamped .txt file and upload to GCS
17. Upload filtered train/test DataFrames (selected features + target) to BigQuery
"""
import sys
import os
import argparse
...
def main():
    ...
    destination_table_train = f"{gcp['gcp_project']}.{gcp['gcp_db']}.{gcp['prefix']}_selected_features_train_{timestamp}"
    ...
    destination_table_test = f"{gcp['gcp_project']}.{gcp['gcp_db']}.{gcp['prefix']}_selected_features_test_{timestamp}"
