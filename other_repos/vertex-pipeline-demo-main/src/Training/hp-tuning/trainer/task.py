...
def train_model(args) -> Dict[str, Any]:
    ...
    dtrain = xgb.DMatrix(X_train_np, label=y_train.to_numpy(copy=False))
    dtest  = xgb.DMatrix(X_test_np,  label=y_test.to_numpy(copy=False))
    ...
    print("\n6. Verifying model properties...")
    feature_names = bst.feature_names
    if feature_names:
        print(f"   ⚠️  Warning: Model has feature names (unexpected)")
    else:
        print(f"   ✓ Model has NO feature names (as expected)")
        print(f"   ✓ Model uses positional features (order matters)")
    ...
    timestamp = os.getenv('timestamp', 'test')
    ...
    model_output_dir = f"{model_dir}/hp_model_output/{timestamp}"
