...
from pyspark.sql.functions import to_date, col, udf, when, lit 
...
# Initialize Spark Session with BigQuery connector and memory config
spark = SparkSession \
  .builder \
...
spark.conf.set("project", "anbc-dev-hcm-cm-de")
spark.conf.set("temporaryGcsBucket", "hcm-cm-de-code-hcb-dev")
...
print("Reading Training data from BigQuery...")
train_df = spark.read \
    .format("bigquery") \
    .option("query", process_sql_file(sql, constants)) \
    .option('project', 'anbc-dev-hcm-cm-de') \
    .option('temporaryGcsBucket', 'hcm-cm-de-code-hcb-dev') \
    .option('viewsEnabled', 'true') \
    .option('materializationProject', 'anbc-hcb-dev') \
    .option('materializationDataset', 'cm_medicaid_hcb_dev') \
    .load()
...
# Drop Spark DataFrame to free memory
train_df.unpersist()  # Remove from Spark cache
del train_df          # Delete Python reference
print("✅ Training Spark DataFrame dropped")

test_df = spark.read \
    .format("bigquery") \
    .option("query", process_sql_file(sql2, constants)) \
    .option('project', 'anbc-dev-hcm-cm-de') \
    .option('temporaryGcsBucket', 'hcm-cm-de-code-hcb-dev') \
    .option('viewsEnabled', 'true') \
    .option('materializationProject', 'anbc-hcb-dev') \
...
spark.conf.set("spark.sql.execution.arrow.pyspark.enabled", "false")
spark.conf.set("spark.sql.execution.arrow.pyspark.fallback.enabled", "true")
df2 = test_df.toPandas()
print("Test data shape: ", df2.shape)

# Drop Spark DataFrame to free memory
test_df.unpersist()  # Remove from Spark cache
del test_df          # Delete Python reference
print("✅ Test Spark DataFrame dropped")
...
with open('gdm_selected_features_pyspark.txt', 'w') as file:
...
destination_path = "vertex-test/codecode/gdm_selected_features_pyspark.txt"

upload_to_gcs(bucket_name, "gdm_selected_features_pyspark.txt", destination_path)
...
try:
    ...
    train_df_selected_spark = spark.createDataFrame(train_df_selected)

    train_df_selected_spark.write \
        .format("bigquery") \
...
try:
    ...
    test_df_selected_spark = spark.createDataFrame(test_df_selected)

    test_df_selected_spark.write \
        .format("bigquery") \
        .option("table", destination_table_test) \
        .option("project", "anbc-dev-hcm-cm-de") \
        .option("temporaryGcsBucket", "hcm-cm-de-code-hcb-dev") \
        .option("writeMethod", "direct") \
        .mode("overwrite") \
